...
from IFKD.engine import train_one_epoch, test
...
def get_args():
    ...
    parser.add_argument('--train_data_path', default='/home/user/desktop/dsm/Dataset/HR_Image_dataset', type=str)
    parser.add_argument('--test_data_path', default='/home/user/desktop/dsm/Dataset/kodak_test', type=str)
    parser.add_argument('--resume', default='/home/user/dsm/project/IFKD/IFKD/outputs/CAKD_swin2res_snr7_AWGN/checkpoint-1999.pth', help='resume from checkpoint')
    parser.add_argument('--num_workers', default=10, type=int)

    # model
    parser.add_argument('--model_s', type=str, default='resnet_small_C128', metavar='MODEL', )
    parser.add_argument('--model_t', type=str, default='swinjscc_base_C128', metavar='MODEL', )
    parser.add_argument('--model_at', type=str, default='resnet_AT_C128', metavar='MODEL', )
    parser.add_argument('--model_t_path',
                        default='/home/user/dsm/project/IFKD/outputs/jscc_swin_base_C128_snrR/checkpoint-1999.pth',
                        help='resume from checkpoint')
    parser.add_argument('--model_s_path',
                        default='',
                        help='resume from checkpoint')
    parser.add_argument('--model_a_path',
                        default='/home/user/dsm/project/IFKD/IFKD/outputs/at_swin2res_snr7/checkpoint-1999.pth',
                        help='resume from checkpoint')
...
def main(args):
    ...
    embed_dim = f_s.shape[-1] if f_s.size().__len__() == 3 else f_s.shape[1]

    projector = create_projector(f_s.size(), f_s.size())
    g_nce = nn.Sequential(
    ...
    projector_x = create_projector(data.size(), f_s.size())
    ...
    # load parameters
    if args.model_s_path:
        checkpoint_s = torch.load(args.model_s_path, map_location='cpu')
        checkpoint_model_s = checkpoint_s['model']
        model_s.load_state_dict(checkpoint_model_s)
    if args.model_t_path:
        checkpoint_t = torch.load(args.model_t_path, map_location='cpu')
        checkpoint_model_t = checkpoint_t['model']
        model_t.load_state_dict(checkpoint_model_t)
    if args.model_a_path:
        checkpoint_a = torch.load(args.model_a_path, map_location='cpu')
        checkpoint_model_a = checkpoint_a['model']
        model_a.load_state_dict(checkpoint_model_a)
    ...
    cur_lr = args.lr
    model_params = [
        {'params': model_s.parameters(), 'lr': cur_lr},
        {'params': model_a.encoder.parameters(), 'lr': cur_lr, 'lr_scare': 0.1},
        {'params': projector_list.parameters(), 'lr': cur_lr}
    ]
    optimizer = optim.Adam(model_params, lr=cur_lr)
    load_model(args, model_s, optimizer, projector_list)

    for _, p in model_t.named_parameters():
        p.requires_grad = False
    for _, p in model_a.decoder.named_parameters():
        p.requires_grad = False
    for _, p in model_s.named_parameters():
        p.requires_grad = True
    for _, p in model_a.encoder.named_parameters():
        p.requires_grad = True
    for _, p in projector_list.named_parameters():
        p.requires_grad = True

    if not args.eval:
        for epoch in tqdm(range(args.start_epoch, args.epochs), ncols=50):
            train_one_epoch(args, model_a, model_s, model_t, projector_list, train_loader, optimizer, epoch, logger)
            if (epoch + 1) % args.save_model_freq == 0 or (epoch+1) == args.epochs:
                save_model(args, epoch, model_s, optimizer, projector_list)
            if (epoch + 1) % args.test_model_freq == 0:
                test(args, model_s, test_loader, logger)
    else:
        test(args, model_s, test_loader, logger)
